...
export default class Row extends PureComponent<any, any> {
  ...
  handlePayInvoice = async () => {
    const {
      invoiceData: { type, toAddress: address, amount, destAddress, fromAddress },
      invoiceData: invoice,
    } = this.props
    ...
    if (walletData) {
      const { currency, balance, unconfirmedBalance } = walletData
    ...
    return (
      <div id="historyRowAmountInfo">
        {direction === directionType ? (
          <div styleName="amount" style={{ display: 'flex', flexDirection: 'column' }}>
            {`+ ${parseFloat(Number(value).toFixed(5))}`} {type.toUpperCase()}
            {standard ? <span styleName="tokenStandard">{standard.toUpperCase()}</span> : ''}
            {txType === 'INVOICE' ? (
              <span styleName="smallTooltip">
                <Tooltip id="RowTooltipInvoice">Invoice</Tooltip>
              </span>
            ) : (
              ''
            )}
  ...
  returnLinkRouter = (params) => {
    let { location, targetPath, tokenPart, name, hash } = params
  ...
  render() {
    ...
    const fiatValue = exCurrencyRate
    ...
    let invoiceStatusText = (
      <FormattedMessage id="HistoryRowInvoiceStatusNew" defaultMessage="Пока не оплачен" />
    )
            ...
            <div styleName="historyInfo">
              <div>
                {txType === 'INVOICE' ? (
                  <>
                    ...
                    <div styleName={`${invoiceStatusClass} cell`}>{invoiceStatusText}</div>
                  </>
                ) : (
                  <>
                    <Link
                      to={(location) =>
                        this.returnLinkRouter({
                          location,
                          targetPath: txLink,
                      ...
                      {txType === 'CONFIRM' ? (
                        ...
                        <>
                          {direction === 'in' ? (
                            <FormattedMessage
                              ...
                              values={{
                                address: substrAddress ? (
                                  <span>
                                    <FormattedMessage id="fromRow" defaultMessage="from" />{' '}
                                    {substrAddress}
                                  </span>
                                ) : (
                                  ''
                            ...
                            <FormattedMessage
                              id="RowHistory282"
                              defaultMessage="Sent {address}"
                              values={{
                                address: substrAddress ? (
                                  <span>
                                    <FormattedMessage id="toRow" defaultMessage="to" />{' '}
                                    {substrAddress}
                                  </span>
                                ) : (
                                  ''
                      ...
                      <>
                        {confirmTx.status === 'pending' && (
                          <div styleName="unconfirmed cell">
                            <FormattedMessage
                              id="RowHistory_Confirm_InProgress"
                        ...
                        {confirmTx.status === 'reject' && (
                          <div styleName="confirm red">
                            <FormattedMessage
                              id="RowHistory_Confirm_Rejected"
                        ...
                        {confirmTx.status === 'cancel' && (
                          <div styleName="confirm red">
                            <FormattedMessage
                              id="RowHistory_Confirm_Cancelled"
                      ...
                      <div styleName={confirmations > 0 ? 'confirm cell' : 'unconfirmed cell'}>
                        {confirmations > 0 ? (
                          confirmations > 6 ? (
                            ...
                            <a href="#">
                              <FormattedMessage id="RowHistory341" defaultMessage="Confirmed" />
                            </a>
                          )
                        ) : (
              ...
              {invoiceData && invoiceData.contact && (
                <div styleName="invoiceContactWrapper">
                  <FormattedMessage id="RowHistoryInvoiceContact" defaultMessage="Contact:" />{' '}
                ...
                {confirmTx.isHolder ? (
                  ...
                  <>
                    ...
                    <button onClick={this.handleConfirmTx}>
                      <FormattedMessage id="RowHistory_ConfirmTX_Sign" defaultMessage="Confirm" />
                    </button>
                  </>
                )}
              </div>
